...
(async () => {
  ...
  const run = async (vp, dpr) => {
    ...
    await page.evaluate(([log, k]) => { if (window.__game.setRung) window.__game.setRung(0); window.__game.setSeed(11); window.__game.scriptInput(log); window.__game.setTimeScale(k); window.__game.skipToPhase('act0'); }, [log, timescale]);
    ...
    const out = { hashes: await page.evaluate(() => window.__game.hashLog()), snap: await page.evaluate(() => { const s = window.__game.snapshot(); return { phase: s.phase, rung: s.rung, glyphs: (s.glyphs || []).map(g => g.id + ':' + g.state + ':' + (g.cohN || 0).toFixed(2)) }; }), errors };
  ...
  const rungMismatch = A.snap.rung !== B.snap.rung;
  const ok = cmp.length >= 5 && diff.length === 0 && A.errors.length === 0 && B.errors.length === 0 && !rungMismatch;
  console.log(JSON.stringify({ ok, rung_mismatch: rungMismatch, compared: cmp.length, first_divergence: diff[0] || null, A: A.snap, B: B.snap, errors: { A: A.errors.slice(0, 5), B: B.errors.slice(0, 5) } }, null, 2));
  process.exit(ok ? 0 : 1);
